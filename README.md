...
- **Fullscreen Camera**: a fullscreen webcam playground with visual overlays (`Squares`, `Hex`, `Voronoi`, `Rings`, `Pulse`, `Tip Ripples`, `Tip Ripples v2`, `Static`) plus webcam-backed games (`Hand Bounce`, `Brick Dodger`, `Breakout Co-op`, `Breakout`, `Finger Pong`, `Slice Air`, `Invaders`, `Flappy`, `Missile Command`).
